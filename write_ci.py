    ...
    'jobs': {
        'test-agent': {
            ...
            'steps': [
                ...
                {'name': 'Run Agent tests', 'run': 'cd ruflo-agent && python -m pytest tests/ -v'}
            ]
        },
        'test-nemoclaw': {
            'runs-on': 'ubuntu-latest',
            'steps': [
                ...
                {'name': 'Run NemOC law tests', 'run': 'cd nemoclaw && python -m pytest tests/ -v || echo "No tests found"'}
            ]
        },
        'build-docker': {
            'runs-on': 'ubuntu-latest',
            'steps': [
                {'uses': 'actions/checkout@v4', 'with': {'path': '.'}},
                {'name': 'Build Docker stack', 'run': 'cd ruflo-os && docker compose -f docker/docker-compose.yml build || echo "Docker build failed (non-critical)"'}
            ]
        },
        'e2e-tests': {
            'runs-on': 'ubuntu-latest',
